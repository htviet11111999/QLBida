...
public class KhongGian extends AppCompatActivity {
    Button btn_tieptuc, btn_thoat;
    RadioGroup rd_khonggian;
    TextView ketqua;
    String sdt ;


    public ArrayList<RongRai> kt = new ArrayList<>();
    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.chatbot_khonggian);
        Intent intent = getIntent();
        sdt= intent.getStringExtra("number");

        ketqua = (TextView) findViewById(R.id.ketqua_khonggian);
        rd_khonggian = (RadioGroup) findViewById(R.id.rd_khonggian);
        btn_tieptuc = (Button) findViewById(R.id.btn_tt_chatbot);
        btn_thoat = (Button) findViewById(R.id.btn_thoatkhonggian);

        rd_khonggian.setOnCheckedChangeListener(new RadioGroup.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(RadioGroup radioGroup, int i) {

                switch (i){
                    case R.id.radio_rongrai:
                        ApiService.apiService.layDSRongRai()
                                .enqueue(new Callback<ArrayList<RongRai>>() {
                                    @Override
                                    public void onResponse(Call<ArrayList<RongRai>> call, Response<ArrayList<RongRai>> response) {
                                        kt = response.body();
                                    ...
                                    @Override
                                    public void onFailure(Call<ArrayList<RongRai>> call, Throwable t) {

                                    }
                                });
                        break;
                    case R.id.radio_binhthuong:
                        ApiService.apiService.layDSRongRai()
                                .enqueue(new Callback<ArrayList<RongRai>>() {
                                    @Override
                                    public void onResponse(Call<ArrayList<RongRai>> call, Response<ArrayList<RongRai>> response) {
                        ...
                        break;
                    case R.id.radio_khongquantam:
                        ApiService.apiService.layDSRongRai()
                                .enqueue(new Callback<ArrayList<RongRai>>() {
                                    @Override
                                    public void onResponse(Call<ArrayList<RongRai>> call, Response<ArrayList<RongRai>> response) {
                                        kt = response.body();
                                        RongRai p = new RongRai();
                                        p.setSoluong(0);
                                                            ...
                                                            public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                                                                ApiService.apiService.xoaBaiGiuXe()
                                                                        .enqueue(new Callback<JsonObject>() {
                                                                            @Override
                                                                            public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                                                                                Intent intent = new Intent(KhongGian.this, Menu_KH.class);
                                            ...
                                            public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                                                ApiService.apiService.xoaSLNguoi()
                                                        .enqueue(new Callback<JsonObject>() {
                                                            @Override
                                                            public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                                                                ApiService.apiService.xoaBaiGiuXe().enqueue(new Callback<JsonObject>() {
                                                                    @Override
                                                                    public void onResponse(Call<JsonObject> call, Response<JsonObject> response) {
                                                                        Intent intent = new Intent(KhongGian.this, MainChatBot.class);
                                                                        intent.putExtra("number", sdt);
                                                                        startActivity(intent);
                                                                    }
    ...
    public String KNN(ArrayList<RongRai> r, int n,  RongRai p) {
        for (int i = 0; i < n; i++) {
